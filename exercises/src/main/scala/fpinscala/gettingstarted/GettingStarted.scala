...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @tailrec
    def fibInner(n: Int, prev: Int, cur: Int): Int = {
      if (n == 0) prev
      else fibInner(n-1, cur, cur + prev)
    }
    fibInner(n, 0, 1)
  }
...
object TestIsSorted {
  def main(args: Array[String]): Unit = {
    Int
    def lt = (a: Int, b: Int) => a <= b : Boolean
    assert(PolymorphicFunctions.isSorted(Array(1,2,3), lt))
    assert(!PolymorphicFunctions.isSorted(Array(1,1,3,2,3,4,5), lt))
  }
}